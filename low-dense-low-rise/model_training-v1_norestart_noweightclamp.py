...
model_dir = current_file_dir / 'models_v1'
img_dir = current_file_dir / 'imgs_v1'
...
for weight in weights:
    ...
    warmup = LinearLR(optimizer, start_factor=0.7, end_factor=1.0, total_iters=warmup_epochs)    
    cosine = CosineAnnealingLR(optimizer, T_max=num_epochs - warmup_epochs, eta_min=1e-5 )    
    scheduler = SequentialLR(optimizer, schedulers=[warmup, cosine], milestones=[warmup_epochs])

    log_dir = f"norestart_noweightclamp_{weight}"
